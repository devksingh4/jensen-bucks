...
std::set<std::string> get_cities(char *filepath)
{

    std::ifstream weather_file(filepath);
    std::string line;
...
int main(int argc, char *argv[])
{
    ...
    nvtxRangePushA("Total Execution");
    ...
    nvtxRangePushA("Construct Map");
    std::set<std::string> all_cities = get_cities(argv[1]);
    ...
    nvtxRangePushA("Cuda Mallocs + Memcpys (1)");
    ...
    for (int b = 0; b < num_parts; b += batch_size)
    {
        ...
        nvtxRangePushA("Memcpy for Batch");
